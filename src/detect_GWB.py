...
# Model the power-spectra:
    ...
    for pair in pairs:
        ...
        if saveCVM or writeSpec:
            ...
            f=0
            while f < len(freq):
                fle.write("%g %g %g %g %g %g %g\n"%(freq[f],xspec[p][1][f],xspec[p][3][f],xspec[p][4][f],mp1[f],mp2[f],A2_guess*Pgn[p][f]*FfF[f]))
                f+=1
            fle.close()
    ...
    while p < np:
        ...
        while f < mx:
            ...
            if f==1:
                factor=0.9
            A2ijk.append(xspec[p][1][f]/zeta[p]/Pgn[p][f]/factor) 
            i+=1
            f+=1
        p+=1
        ...
        while a < N:
            ...
            while b <= a:
                ...
                if delta > 2*bw:
                    b+=1
                    continue
                X=crossCovar(delta,Ts[ij][0],Ts[ij][0],bw)
                #psr1=pairs[ij][0]
                #psr2=pairs[ij][1]
                #psr3=pairs[lm][0]
                #psr4=pairs[lm][1]
                #print X,psr1,psr2,psr3,psr4,f1,f2,T1,T2
                ...
                if ij==lm:
                    ...
                    C = 0.5 * (P1*P2)/pow(zeta[ij]*P_gw_nrm(fa)*factor,2)
                    covar[a][b] += X*C
                    if a!=b:
                        covar[b][a] += X*C
                C=GWcovar[ij][lm]
